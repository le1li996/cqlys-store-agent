...
// 对话面板：消息流 + 输入框 + 工具栏 + 真实文件上传
export default function ChatPanel({ module, messages, loading, onSend }) {
  const [text, setText] = useState('')
  const [online, setOnline] = useState(false)
  const [memory, setMemory] = useState(true)
  const [pending, setPending] = useState([]) // 待发送文件
    ...
    <section className="chat-panel">
      ...
      <div className="chat-input-bar">
        <div className="chat-tools">
          ...
          />
          <button
            className={`tool toggle ${online ? 'on' : ''}`}
            onClick={() => setOnline((v) => !v)}
          >
            🌐 联网{online ? '·开' : ''}
          </button>
          <button
            className={`tool toggle ${memory ? 'on' : ''}`}
        ...
        <div className="chat-input-row">
          <textarea
            ...
            rows={2}
          />
          <button className="send-btn" onClick={send} disabled={loading || (!text.trim() && pending.length === 0)}>
            发送 ↑
          </button>
        </div>
      </div>
